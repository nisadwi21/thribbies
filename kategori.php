...
<head>
  <meta charset="UTF-8" />
  <title>Kategori <?= htmlspecialchars($kategori) ?> | Thribbies</title>
...
<div class="judul-kategori">
  <h2>Kategori: <?= htmlspecialchars($kategori) ?></h2>
</div>
<!-- Produk Grid -->
  ...
  <?php if (mysqli_num_rows($query) > 0): ?>
    <?php while ($produk = mysqli_fetch_assoc($query)) : ?>
      <div class="produk-card">
        <img src="imgs/<?= $produk['gambar'] ?>" alt="<?= $produk['nama'] ?>">
        <h3><?= $produk['nama'] ?></h3>
        <p>Rp <?= number_format($produk['harga'], 0, ',', '.') ?></p>
        <p><?= $produk['lokasi'] ?></p>
        <a href="detail.php?id=<?= $produk['id'] ?>">Detail</a>
      </div>
    <?php endwhile; ?>
  <?php else: ?>
